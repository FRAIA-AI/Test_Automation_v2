...
class FnxUploadFailed(MonitorFailure):
    def __init__(
        self,
        message: str,
        phase: str,
...
class FnxPage:
    ...
    def upload_to_patient_form(
        ...
        try:
            ...
            raise FnxUploadFailed(
                message=(
                    "FNX parser upload dialog did not close "
                    "after submitting the file."
                ),
                phase="fnx_parser_upload",
            ) from error

    def expect_parsed_patient(
        self,
        oracle: dict,
    ) -> None:
        cpr = self.page.get_by_placeholder(
            re.compile(
                r"CPR",
                re.IGNORECASE,
            )
    ...
    def open_analytics(
        ...
        upload_controls = self.page.locator(
            'input[type="file"]'
        )

        upload_text = self.page.get_by_text(
            re.compile(
                r"Upload file|"
                r"Upload fil|"
                r"Vælg fil|"
        ...
        while time.monotonic() < deadline:
            if upload_controls.count() > 0:
                return

            if (
                upload_text.count() > 0
                and upload_text.is_visible()
            ):
                return
        ...
        raise AssertionError(
            "FNX Analytics opened, but no file-upload "
            "control became available within 30 seconds."
        )

    def upload_to_analytics(
        self,
        file_path: Path,
        patient_name: str,
    ) -> None:
        ...
        uploading = self.page.locator(
            "span, p, div"
        ).filter(
            has_text=re.compile(
                r"^\s*(?:UPLOADING|UPLOADER)\s*$",
                re.IGNORECASE,
            )
        )
        ...
        uploaded = self.page.locator(
            "span, p, div"
        ).filter(
            has_text=re.compile(
                r"^\s*(?:UPLOADED|UPLOADET)\s*$",
                re.IGNORECASE,
            )
        )
    ...
    def _wait_for_expected_content(
        ...
        *,
        ...
        raise AssertionError(
            f"{label} did not contain any expected term "
            f"{expected_any!r} within {timeout_ms} ms."
        )
    ...
    def _wait_for_new_expected_content(
        ...
        *,
        ...
        raise AssertionError(
            f"{label} did not add any expected term "
            f"{expected_any!r} within {timeout_ms} ms."
        )
