...
from tests.pages.consultation_page import ConsultationPage
from tests.pages.dashboard_page import DashboardPage, SyntheticPatient


@pytest.mark.regression
def test_direct_consultation_generates_and_saves_valid_note(app_session_factory) -> None:
    settings = get_settings()
    require_admin_credentials(settings)
    require_files(settings.audio_file, settings.audio_oracle_file)
    oracle = load_oracle(settings.audio_oracle_file)
    ...
    consultation_id = dashboard.start_direct_consultation(patient)
    upload_consultation_audio(
        session.context,
    ...
    wait_for_processed_transcription(
        ...
        consultation_id=consultation_id,
        auth_cookie_name=settings.auth_cookie_name,
        expected_any=list(oracle.get("transcription_expected_any", [])),
    )

    consultation = ConsultationPage(session.page)
    consultation.finish_and_open_note()
    consultation.wait_for_valid_note(oracle)
    consultation.approve_note()
    consultation.submit_feedback(rating=10)
    dashboard.expect_recent_consultation(patient)
